...
<br>
**Details**: Melting temperatures (Tm) are calculated using the nearest-neighbor method (see `r ?rprimer::find_oligos` for details).
<br><br>
```{r}
...
custom_table(
  ...
  col.names = c(
    ...
    "Tm (majority)", "Sense", "Perfect matches (majority)", 
    "Perfect matches (IUPAC)"
  )
)
}
```
...
```
<br><br>
**Fig. 3** Matching and mismatching sequences. 
