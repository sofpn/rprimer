...
test_that("majority_consensus returns error when it should", {
  expect_error(majority_consensus(unclass(example_rprimer_sequence_profile)))
})

test_that("majority_consensus works", {
  cons <- majority_consensus(example_rprimer_sequence_profile)
  expect_equal(length(cons), ncol(example_rprimer_sequence_profile))
...
test_that("iupac_consensus returns an error when it should", {
  expect_error(
    iupac_consensus(example_rprimer_sequence_profile, threshold = 3)
  )
  expect_error(
    iupac_consensus(example_rprimer_sequence_profile, threshold = NA)
  )
  expect_error(
    iupac_consensus(unclass(example_rprimer_sequence_profile))
  )
})

test_that("iupac_consensus works", {
  cons <- iupac_consensus(example_rprimer_sequence_profile, threshold = 0.2)
  expect_true(is.character(cons))
  expect_equal(length(cons), ncol(example_rprimer_sequence_profile))
  cons2 <- example_rprimer_sequence_profile[1:2, ]
  cons2 <- new_rprimer_sequence_profile(cons2)
  expect_warning(iupac_consensus(cons2, threshold = 0.2))
})

test_that("gap_frequency returns an error when it should", {
  expect_error(gap_frequency(unclass(example_rprimer_sequence_profile)))
})

test_that("gap_frequency works", {
  gaps <- gap_frequency(example_rprimer_sequence_profile)
...
test_that("nucleotide_identity returns an error when it should", {
  expect_error(nucleotide_identity(unclass(example_rprimer_sequence_profile)))
})

test_that("nucleotide_identity works", {
  identity <- nucleotide_identity(example_rprimer_sequence_profile)
...
test_that("shannon_entropy returns an error when it should", {
  expect_error(shannon_entropy(unclass(example_rprimer_sequence_profile)))
})

test_that("shannon_entropy works", {
  entropy <- shannon_entropy(example_rprimer_sequence_profile)
...
test_that("sequence_properties returns an error when it should", {
  expect_error(
    sequence_properties(unclass(example_rprimer_sequence_profile))
  )
  expect_error(
    sequence_properties(example_rprimer_sequence_profile, iupac_threshold = 3)
  )
  expect_error(
    sequence_properties(example_rprimer_sequence_profile, iupac_threshold = NA)
  )
})

test_that("sequence_properties works", {
  properties <- sequence_properties(example_rprimer_sequence_profile)
  expect_s3_class(properties, class = "rprimer_sequence_properties")
  expect_equal(
    colnames(properties),
