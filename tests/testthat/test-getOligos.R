...
test_that(".getNmers works", {
    ...
    toTest <- exampleRprimerProfile$majority[1:50]
...
test_that(
  ...
  and .filterOligos work",
    {
        data("exampleRprimerProfile")
        toTest <- .generateOligos(as.data.frame(exampleRprimerProfile))
        ...
        toTest <- .filterOligos(
            toTest,
            gcClamp = FALSE, avoid5EndG = TRUE, avoid3EndRuns = FALSE,
            minEndIdentity = 0.99
        )
        ...
        toTest <- .filterOligos(
            toTest,
            gcClamp = FALSE, avoid5EndG = TRUE, avoid3EndRuns = TRUE
        )
