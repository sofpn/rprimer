...
* `temperature` Annealing temperature (in Celcius), for calculation of delta G. Delta G is calculated for perfectly matching oligo-target duplexes using the nearest neighbor method developed by [@SantaLuciaUnified]. Formula, salt correction method and table values are from [@santalucia2004thermodynamics]
...
* `tmDiffPrimersProbe`: Acceptable tm difference between the primer pair and probe, defaults to `c(0, 20)`. The tm difference is calculated by subtracting the mean tm of the probe with the mean tm of the primers
...
The workflow presented here covers several important steps for primer and PCR assay design, but not all. Two important, additional steps are: 1) checks for the potential of hairpin and primer-dimer formation and: 2) *in silico* checks for exclusivity, by matching the primer and probe set to non-targets. Below, I present a few examples on how to export the results to file, so that they can be analyzed further by other software. 

**Oligos and assays to fasta format**

The following functions can be used for converting all sequence variants of all oligos or assays to a DNAStringSet-object. The `revAsRc` option regulates whether reverse primers should be written as reverse complements or not. 
...
assaysToDNAStringSet <- function(x, revAsRc = TRUE) { 
  ...
  rev <- if (revAsRc) {
    x$sequenceRev 
      ...
      lapply(x$sequenceRev, function(y) {
        y <- strsplit(y, split = "")
        y <- do.call("rbind", y)
        y <- rprimer:::.reverseComplement(y)
  ...
  Biostrings::DNAStringSet(c(fwd, rev, pr, prRc))  
}
...
This object can then easily be exported in fasta-format, for use in software for e.g. multiple sequence alignment or dimer check. 
