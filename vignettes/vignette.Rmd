...
The aim of rprimer is to provide tools for analyzing sequence conservation and designing degenerate primers and probes for sequence variable viruses. A multiple DNA sequence alignment with target sequences is used as input and the outputs consist of data frames (tabular data) and dashboard-like plots. 
...
Once the alignment is imported, the first step is to generate a consensus profile, which contains the following information: 
...
* Identity, which is the proportion of sequences, among all sequences with a DNA base (i.e., A, C, G or T), that has the majority consensus base
* IUPAC consensus character (ambiguous base). It includes all DNA bases that occurs at a proportion higher than the user-specified "ambiguity threshold". The threshold can range from 0 to 0.2. A threshold of 0 (default) will catch all the variation, but with the potential cost of generating oligos with high degeneracy. A higher threshold will catch most of the variation, but with the potential cost of missing minor sequence variations (this information is provided by the coverage, see below)
...
* Coverage, which describes the proportion of sequences in the target alignment, among all sequences with a DNA base, that are covered the IUPAC consensus character. 
...
* `minThreeEndCoveragePrimer`: minimum allowed coverage at the 3' end (the last five bases). If set to `1`, all bases of the 3' end must be complementary to all sequence variants in the target alignment. Defaults to `0.98` 
* `gcRangePrimer`: GC-content range of primers (proportion), defaults to `c(0.40, 0.65)`
* `tmRangePrimer`: melting temperature range for primers, defaults to `c(55, 65)`. Melting temperatures are calculated for perfectly matching oligo-target duplexes using the nearest neighbor method [@SantaLuciaUnified], using formula, salt correction method and table values as described in [@santalucia2004thermodynamics]. See the man page (`?oligos`) for more information 
...
The next step is to find pairs of forward and reverse primers. Probes will be added to the assays if probes were designed in the previous step. 

The `assays()` function has only two arguments:   
...
There are now over 1000 assay candidates, but the number can be reduced by subsetting the dataset to only contain the assays with the best score(s). The assay score is the summarized oligo score. Thus, the best possible score is 0 and the worst possible score is 24 for assays with only primers and 36 for assays with probes (since the maximum possible score for each oligo is 12, see Step 2: `oligos`). 
...
Before proceeding to wet lab evaluation, the primer and probe candidates must be evaluated for the potential to form primer-dimers and hairpin structures and the potential to cross react with non-targets.
...
The `asRC` option regulates whether reverse primers and minus sense probes should be written as reverse complements or not. Defaults to `TRUE`. 

The `asIUPAC` option regulates whether the oligos should be presented in IUPAC-format or not. Defaults to `TRUE`. If set to `FALSE`, it will instead include all sequence variants of each oligo. 
...
convertToDNAStringSet(myAssays[1, ], asRc = FALSE, asIUPAC = FALSE)
```
...
## Design primers, probes and assays (modify settings if wished) 
myConsensusProfile <- consensusProfile(myAlignment, ambiguityThreshold = 0.05)
myOligos <- oligos(myConsensusProfile)
myAssays <- assays(myOligos)
