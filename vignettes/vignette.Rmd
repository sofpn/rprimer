...
If the degeneracy is to be kept low and the target sequence heterogeneous, it is next to impossible to fully avoid mismatches between the template and primers/probe. The effect of a mismatch depends to a great extent on its location. For primers, mismatches at the 3’-end can greatly reduce amplification efficiency and should best be avoided, while mismatches at the 5’-end are less detrimental, and can therefore be tolerated to some extent. However, mismatches within the binding region of a hydrolysis probe should preferably be avoided. Especially 5’-end mismatches can be particularly severe, since the 5’-3’ exonuclease activity of the DNA polymerase relies on completely hybridized nucleotides to hydrolyze DNA duplexes. 

rprimer was developed to implement previously developed primer and probe design guidelines for sequence variable targets. It takes a multiple sequence alignment with target sequences of interest as input and designs primers, probes and PCR assays from user specified design criteria. 
...
The target sequences were collected by searching for “[a-z]”[porgn:__txid1678143] (the taxid of Ortohepevirus A, i.e., HEV) on the nucleotide database of NCBI GenBank, and filtering for sequence lengths > 6000, to get complete and nearly complete genomes. The search had 674 hits (as of January 19, 2020). The sequences were downloaded (as complete record, fasta-file) and run through Hepatitis E virus genotyping tool version 0.1 (https://www.rivm.nl/mpf/typingtool/hev/, accessed January 19, 2020) to verify that they were classified as Ortohepevirus A. All sequences that were not classified as Ortohepevirus A were removed, and the remaining sequences (n = 642) were reduced to 200 by random selection, to not provide a too large example dataset for this package. The sequences were then aligned using the online version of mafft version 7 (https://mafft.cbrc.jp/alignment/server/, accessed January 19, 2020), using Auto settings. 
...
Once the alignment is imported, the first step is to generate a consensus profile. The following information is retrieved at each position in the alignment:
...
* Coverage, which describes the total proportion of bases that are "covered" by the IUPAC consensus character. Gaps and ambiguous bases in the alignment are not included in this calculation 

`consensusProfile()` takes a `Biostrings::DNAMultipleAlignment` object as input and returns an `RprimerProfile` object, which contains all the information needed to design primers and probes. Positions and sequences that are masked in the `DNAMultipleAlignment` will be removed.
...
The dots represent the value at each position and the black lines represent centered running averages. High identity values and low entropy values indicate high sequence conservation. The HEV genome contains very few conserved regions. 
...
The nucleotide distribution can be plotted (preferably on a short range), by specifying `type = nucleotide`: 
...
The design process starts by generating all possible oligos of the user specified primer and probe lengths from the IUPAC consensus sequence. For each oligo, information is also collected on start and end position, degeneracy, gap frequency, majority consensus sequence and coverage. All oligos with degeneracies and gap frequencies higher than the user specified criteria are removed.  

For primers, there are two different strategies that can be used at this point. Type = mixed ..................

Then, the next step is to generate all possible sequence variants of each oligo, and calculate GC-content and melting temperature, and to collect information on the presence of GC-clamp, three-end runs, and mono- and dinucleotide repeats for each variant. Mean values and ranges on GC-content and melting temperature are also calculated.    

Then, all possible sequence variants of each oligo are generated, and 

* Coverage 

coverage, 
end coverage: philosophy is similar to CODEHOP (REF), which allows ... relax degeneracy? diff settings on 3 and 5' ? use majority instead? /include majority as well?????
tm
...
To provide reproducible examples, I've also included an alignment of class `Biostrings::MultipleDNAAlignment`. It is loaded by `data("exampleRprimerAlignment")`.
