...
Primer and probe design are central parts of PCR assay development. Successful amplification and detection depend on several factors, such as optimal primer and probe length, GC-content, melting temperature and sequence complexity. Primer and probes must be designed so that formation of dimers and hairpin structures are avoided, and the intended target region must be sufficiently conserved to avoid severe mismatches between the primers/probe and template. This together makes PCR assay design a challenging task, which is not least reflected by the large number of computational methods available. One of the most comprehensive and widely used tools is Primer3. However, one important limitation with Primer3 is that the design process is based on a single sequence, which means that primers can be inadvertently placed in sequence heterogenous regions, resulting in amplification and quantification bias. This can, in part, be circumvented by making a “consensus profile”  from an alignment of the target sequences of interest, and masking the variable regions in the consensus sequence before the design process. However, for many applications, it can be difficult to find even a few completely conserved regions that are suitable as primer or probe targets.

A common approach for amplifying sequence diverse targets is to use degenerate primers. A degenerate primer has one or more positions with several possible bases, and the degeneracy refers to the number of unique sequence variants it encompasses. These ambiguous bases are named according to a coding system determined by IUPAC. For instance, the IUPAC consensus character K can be either G or T, and N can be A, C, G or T. For some applications, it is not uncommon to use primers with up to several hundreds or thousands of sequence variants. However, the amplification efficiency may reduce as the degeneracy increases, because only a fraction of the primers will match perfectly to the target sequence and the synthesized product. The likelihood of interaction with non-targets or other primer variants also rises, which risks to reduce the specificity of the assay. An optimal design strategy would therefore be to find primer and probe binding regions with as low degeneracy as possible. 

If the degeneracy is to be kept low and the target sequence heterogeneous, it is next to impossible to fully avoid mismatches between the template and primers/probe. The effect of a mismatch depends to a great extent on its location. For primers, mismatches at the 3’ end can greatly reduce amplification efficiency and should best be avoided, while mismatches at the 5’ end are generally less detrimental. 

Mismatches within the binding region of a hydrolysis probe can also be severe, since the 5’-3’ exonuclease activity of the DNA polymerase relies on completely hybridized nucleotides to hydrolyse DNA duplexes. 

This R package ("rprimer") was developed to implement commonly adopted primer and probe design guidelines for sequence variable targets, such as RNA viruses. It takes a multiple sequence alignment with target sequences of interest as input and designs primers, probes and PCR assays from user specified design criteria. 
...
* The **mixed strategy** is based on the Consensus-Degenerate Hybrid Oligonucleotide Primer (CODEHOP) principle. It generates primers from both the majority and the IUPAC consensus sequence, which consist of a shorter degenerate part at the 3' end (~1/3 of the primer) and a longer consensus part at the 5' end (~2/3 of the primer). This means that the 5' end will lack ambiguous bases, and instead contain the most probable nucleotide at each position. The purpose of this strategy is . Mismatches less detrimental at the 5' end.   
Mismatching primer-template duplexes have lower melting temperatures than their perfectly matching equivalents, which means that the tolerability to mismatches can increase if the annealing teperature drops  - but risk of non speficic interaction 
 However, consensus primers may be too dissimilar to an unknown target to efficiently anneal to the original template, and degenerate primers may be too dissimilar to each other to efficiently amplify the synthesized product.*

Long primers (> 25 bases) will better accomodate mismatches due to their higher melting temperature. 

Probes are always designed using the ambiguous strategy. 

**Design settings**

`oligos()` has the following arguments that can be specified by the user:  
...
To run the design process with default settings, it is only the first argument (`x`) that must be specified, i.e. the consensus profile from Step 1: 
...
The man page (`?oligos`) contains a detailed description of each variable. What may not be completely self-explanatory is that some variables (`sequence` `gcContent` and `tm`) in fact are lists, where each entry may contain several values. All values within an entry be retrieved by:   
...
I now use the mixed strategy, and design primers only, within a specified region of interest (position 1000 to 4000). I also modify some of the settings: 
...
It is possible to the oligos, to only get the ones with e.g. highest average coverage: 
...
It is also possible to select an oligo and plot the nucleotide distribution of the binding region, by subsetting the consensus profile from Step 1. I do this for the first primer in my selection: 
...
`assays()` has the following arguments that can be specified by the user:  

* `x` the oligos from Step 2.  
...
rprimer covers a lot of steps in the oligo and assay design processes, but not all. Two very important, additional parts are: 1) checks for the potential of hairpin and primer-dimer formation and: 2) in silico checks for exclusivity, by matching the primer and probe set to non-targets. [MER HÄR]
