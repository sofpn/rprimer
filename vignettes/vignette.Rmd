...
The `Rprimer`-classes are extensions of the `DataFrame` class from S4Vectors (Pagès et al., 2020), and behave in a similar manner as traditional data frames (with methods for e.g. [, $, nrow, ncol, head, tail). They can be coerced to traditional such by using `as.data.frame()`. 

Example data sets of each class are provided, and are loaded by: 
...
To provide reproducible examples, I've also included an alignment of class `Biostrings::MultipleDNAAlignment` (Pagès et al., 2020), which is loaded by `data("exampleRprimerAlignment")`.
...
* Identity shows the average identity score of the oligo. A high identity score indicates a conserved target region 
...
The upper plot shows the location of the primer and probe candidates, and the lower plots shows the distribution of the characteristics of all oligo candidates. 
