...
Primer and probe design is the most central part of PCR assay development. Successful amplification and detection depend on several factors, such as optimal primer and probe length, GC-content, melting temperature, sequence complexity and target sequence conservation.

For a set of target sequences of interest, it is often difficult to identify regions that meet all, or even most of these requirements.It is especially developed for finding broadly reactive assays for sequence variable RNA viruses. 

Primer design for RNA viruses can be difficult due to
-main criteria may be to find the most conserved region 
-high sequence variation, few conserved regions  
-with increased ability for ngs -> number of sequences growing -> need for overvrew 


rprimer provides tools for designing (RT)-(q/d)PCR assays from a multiple DNA sequence alignment. Primers and probes are designed from user specified constraints on length, GC-content, melting temperature, maximum allowed degeneracy and target gap frequency. Target regions with low sequence complexity are avoided. For primers, it is also possible to specify a threshold for minimum 3'-end conservation and enforce a GC-clamp, to increase the probability of amplification. Assays can be generated with or without probe, and are designed upon user specified amplicon length and maximum allowed difference in melting temperature between primers, and between the primer pair and probe.  
...
* `getConsensusProfile()`: takes a `Biostrings::MultipleDNAAlignment` object (Pagès et al., 2020) as input and returns .... (organised as an `RprimerProfile` object), which is used as input for;
...
The data can be visualized with `plotData()`, and specific regions can be highlighted using the optional arguments `shadeFrom` and `shadeTo`. The most conserved region of the hepatitis E virus genome seems to be between position 5000-5500:
...
`getOligos()` searches for oligos (primers and probes) from an `RprimerProfile`-object. All oligos are shown in both majority (without degenerate bases) and IUPAC format (with degenerate bases). The following settings can be used for primer and probe design:
...
An error message will return if no oligos are found. If so, a good idea could be to re-run the process from Step 1 and increase the `iupacThreshold` in `getConsensusProfile()` and/or to relax the design constraints in `getOligos()`.
...
* An `NA` indicates that the oligo did not meet the specified design criteria (e.g if a primer lacked GC-clamp when `gcClampPrimer` was set to `TRUE`)
...
* The columns `all` and `allRc` contain all sequence variants of the forward and reverse variant of the primer, respectively. The first oligo candidate is shown here as an example:
...
```{r}
myOligos$gcAll[[1]] # GC content of all variants of the first oligo 
myOligos$tmAll[[1]] # Tm of all variants of the first oligo 
```
...
`getAssays()` finds pairs of forward and reverse primers, and combines them with probes if `probe = TRUE` was used in the previous step. 
...
- tm only takes primer conc and na conc into account
