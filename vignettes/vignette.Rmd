...
The `Rprimer`-classes are extensions of the `DataFrame` class from S4Vectors (Pagès et al., 2020), and behave in a similar manner as traditional data frames (with methods for `[`, `$`, `nrow`, `ncol`, `head`, `tail`, etc.). They can be coerced to such by using `as.data.frame()`. 
...
I import the file and mask all positions with at least 50 % gaps. However, masking positions with high gap frequency should be done with caution, since it may lead to placement of oligos in insert-rich regions. If gap mask is used, a good idea could be to confirm the design process without gap mask. 
...
`getOligos()` searches for primers or primers and probes from an `RprimerProfile`-object. All oligos are shown in both majority (without degenerate bases) and IUPAC format (with degenerate bases). Oligos are designed from the following constraints:  
...
It is of course also possible to use `plotData()` for the assay region:
