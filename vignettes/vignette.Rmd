...
Reverse transcription (RT) PCR based methods are invaluable for both clinical diagnosis and environmental surveillance of viruses. Successful detection of a virus depends on several factors, and primer and probe design is a critical part of RT-PCR assay development [@Bustin]. Poorly designed primers reduces the efficiency of the RT or PCR, which can lead to underestimation of the target concentration or false-negative results. A good primer/probe binds exclusively to the target molecule and does not fold or form stable interactions with other primers/probes. Ideally, it should also bind to a conserved region of the genome, to cover as many sequence variants of the target as possible. However, for many viruses, it can be difficult to find even a few, short conserved regions suitable as primer/probe targets, which means that the sequence variability must be taken into account in the design process. 
...
rprimer seeks to address the challenge of amplifying sequence diverse targets by providing tools for visualizing sequence conservation and generating degenerate primer and probe candidates from a multiple sequence alignment. In this document, I demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR primers, probes and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
* IUPAC consensus character (ambiguous base). Here, the user can specify an “ambiguity threshold” between 0 and 0.2. All bases with a relative frequency higher than that will be included. A threshold of 0 (default) will catch all the variation, but with the potential cost of generating primers and probes with high degeneracy. A higher threshold will catch most of the variation, but with the potential cost of missing minor sequence variations (this information is provided by the coverage, see below)
...
Oligos with sequence variants containing more than four consecutive runs of the same nucleotide (e.g. "AAAAA") and/or more than three consecutive runs of the same di-nucleotide (e.g. "TATATATA") are exclude for consideration. 
...
The man page (`?oligos`) contains a detailed description of each variable. What may not be entirely self-explanatory is that some columns (`sequence` `gcContent`, `tm` and `deltaG`) holds several values if the oligo is degenerate. In such cases, all values within an entry can be retrieved by:   
...
Let's say I want to amplify the region between position 2000 and 3000. I can then "mask" the primer dataset, to not place forward primers after position 2000 and to not place reverse primers after position 3000: 
...
The workflow presented here covers several steps for primer and PCR assay design, but not all. Two important, additional steps are for instance: 1) checks for the potential of primer-dimer and hairpin formation and: 2) *in silico* checks for exclusivity, by matching the primer and probe set to non-targets. 
