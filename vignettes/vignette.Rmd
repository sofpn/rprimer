...
For some applications, such as genotyping, there is often an interest in amplifying a specific region of the genome. In such cases, the alignment can be masked so that only the desired oligo binding regions are available for the subsequent design process. `colMask()` from Biostrings [@PagesBios] can be used for this task: {#Mask}
...
An alignment with masked columns (see `myMaskedAlignment` from the [previous step](#Data) is represented as follows: 
...
As a comparison, I do the same for the dataset where [I masked all positions but 3000 to 4000 and 5000 to 6000](#Mask).
...
* `tmDifferencePrimers`: the maximum allowed difference in tm between the primer pair. Defaults to `NULL`, which means that primers will be paired regardless of their tm. **[A NOTE ON TM DIFF]**
...
My sincere thanks goes to Sebastian Kapell, for providing valuable suggestions on how to improve this package.

# Session info 

The output from `sessionInfo()` from the system I generated this document on is shown below. 
