...
rprimer seeks to address the challenge of amplifying sequence diverse viruses by providing tools for visualizing sequence conservation and designing degenerate oligos^[A degenerate oligo has one or more positions with several possible bases, which are specified by a code system determined by IUPAC. For instance, the IUPAC consensus character K can be either G or T, and N can be A, C, G or T. The degeneracy refers to the number of unique sequence variants encompassed by the oligo.] from a multiple sequence alignment. In this document, I demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR primers, probes and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
