...
**[MASTE SKRIVA TESTER OSV..]**

`checkMatch()` checks how well the generated oligos or assays match to the intended binding region as well as all other regions of the input alignment. The function has two arguments: 

* `x`: an oligo or assay dataset from [Step 2](#Step2) or [3](#Step3)
* `target`: the target alignment used as input in [Step 1](#Step1)

The output gives information of the proportion of oligos that matches perfectly as well as with 1, 2, 3 or 4 or more mismatches to the intended olgio binding region in the input alignment (i.e., on target match). It also reports the proportion of oligos that matches with no more than 4 mismatches to *all other regions* in the input alignment (off target match). Note, however, that the output does not say anything about the type, position or severity of the mismatches.
...
It is also possible to add the match information to the oligo or assay dataset that was used as input, by using `cbind()`:

```{r}
oligoSelection <- cbind(oligoSelection, oligoSelectionMatch[, -1])
```

(the first column (iupacSequence) of `oligoSelectionMatch` can be removed when pasting the two objects together, to not get duplicated columns)
