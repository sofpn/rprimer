...
The ideal oligo binds exclusively to a conserved part of the target of interest and does not fold or form stable interactions (primer dimers) with other oligos, but it is often both difficult and time consuming to identify target regions and oligo combinations that meets all these criteria. Many viruses, especially RNA viruses, evolve quickly and thereby pose significant sequence variability between strains. This makes sequence conservation analysis a key step of the design process, since primers or probes that captures only a part of the target variation may bias the PCR outcome.
...
rprimer seeks to address the challenge of amplyfying sequence variable viruses by providing tools for visualizing sequence conservation and designing degenerate oligos from a multiple DNA sequence alignment. In this document, I describe and demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR oligos and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
For brevity's sake, and to get the alignment to agree more with the average HEV genome length, I masked all positions with a gap frequency of more than 50 %. But keep in mind that gap masking may lead to placement of oligos in insert-rich regions.   
...
Once the consensus profile is generated, oligos can be designed. Primers must be designed and probes are optional. Primers can be generated in two different ways: 
...
Eac oligo is scored according to a simple system, based upon their average identity, coverage, degeneracy, GC-content and tm range. The best possible score is 0 and the worst possible score is 12. Detailed information about the scoring system can be found at the man page (`?oligos`).
...
The man page (`?oligos`) contains a detailed description of each variable. What may not be entirely self-explanatory is that some columns (`sequence` `gcContent`, `tm` and `deltaG`) can hold several values. All values within these entries can be retrieved by:   
...
Now, there are more primer candidates (due to relaxed constraints) and less probe candidates (due to tighter constraints):
...
Sometimes, for instance for sequencing and genotyping applications, the amplicon must span a certain genomic region. To accomplish this, it is possible to "mask" the primer dataset, to not place forward primers after the start position and to not place reverse primers after the end position of the desired amplicon region.

```{r}
## Keep fwd primers that starts before position 2000 and rev primers that starts after position 3000
...
To illustrate assay design with modified settings, I use the masked dataset from Step 2 (`myMaskedSelection`). Recall that I wanted an assay with only primers, that covers position 2000-3000.

```{r}
myAssays2 <- assays(maskedSelection, 
...
The first step is to retrieve the start and end position of the assay, as below:
...
Now, it is possible to highlight the amplicon region, using the argument `highlight` in `plotData()`:

```{r fig 3, fig.width=12, fig.height=6}
plotData(myConsensusProfile, highlight = c(from, to))
```

To get a more detailed view, I can make a subset of the consensus profile, so that it only contains the amplicon region: 
...
These tasks are better performed by other software. Below, I show how to export the results to file so that they can easily be analyzed by other tools.  
...
The "rprimer" classes (`RprimerProfile`, `RprimerOligo` and `RprimerAssay`) are extensions of the `DataFrame` class from S4Vectors [@PagesS4], and behave in essentially the same way as traditional data frames, with methods for `[`, `$`, `nrow`, `ncol`, `head`, `tail`, `rbind`, `cbind` etc. They can be coerced to traditional data frames by using `as.data.frame()`. 
