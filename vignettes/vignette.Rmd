...
Primer and probe design are the most central parts of PCR assay development. Successful amplification and detection depend on several factors, such as optimal primer and probe length, GC-content, melting temperature and sequence complexity. Primer and probes must be designed so that formation of dimers and hairpin loops are avoided, and the intended target region must be sufficiently conserved to avoid severe mismatches between the primers/probe and the template. This together makes PCR assay design a particularly challenging task. 
...
The design workflow is built on three functions: 
...
It is possible to mask positions with high gap frequency by using `Biostrings::maskGaps()`, but this should be done with caution, since it may lead to placement of oligos and amplicons in insert-rich regions. However, In my example, I've applied a gap mask to get the alignment to agree more with the average hepatitis E virus genome length. In case a gap mask is used, a good idea could be to verify the design process without gap mask as well.
...
* `maxDegeneracyPrimer` Maximum number of sequence variants of each primer, defaults to `4`. Note that this does not refer to the *number* of ambigous bases, it is the number of unique sequence variants represented by them ################
* `gcClampPrimer` If primers must have a GC-clamp, defaults to `TRUE`. A GC-clamp is identified as two to three G or C:s within the last five bases (3'-end) of the primer. The presence of a GC-clamp ##################
* `avoidThreeendRunsPrimer` If primers with more than two runs of the same nucleotide at the terminal 3'-end should be avoided, defaults to `TRUE`. ###########################3
...
The `Rprimer`-classes are extensions of the `DataFrame` class from S4Vectors (Pagès et al., 2020), and behave in a similar way as traditional data frames, with methods for `[`, `$`, `nrow`, `ncol`, `head`, `tail`, etc., and can be coerced to traditional data frames by using `as.data.frame()`. 
