...
Reverse transcription (RT) PCR based methods are important cornerstones of both clinical diagnosis and environmental surveillance of viruses. Successful RT-PCR analysis depends on several factors, but primer and probe design is arguably the most critical part of assay development [@Bustin]. Poorly designed primers reduce the efficiency of the RT or PCR and poorly designed probes do not hybridize properly with the target. Both of these can result in underestimation of the target concentration or, in worst case, a false-negative result. A good primer/probe binds exclusively to a conserved region of the target of interest and does not fold or form stable interactions with other primers/probes. Many viruses display great sequence heterogeneity, which makes it difficult to find conserved regions suitable as primer or probe binding sites. In such cases, we must take the sequence variation into account in the design process. 
...
rprimer seeks to address the challenge of amplifying sequence diverse targets by providing tools to visualize sequence conservation and to generate degenerate primer and probe candidates from a multiple sequence alignment. In this document, I demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR primers, probes and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
Below, I show how to export the results to file, so that they can be analyzed further by other software. It is, for instance, important to also check the primer and probe candidates for the potential of primer-dimer and hairpin formation.
