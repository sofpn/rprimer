...
The next step is to design oligos. Primers must be designed and probes are optional. Primers can be generated in two ways: 
...
In the first example, I show how to design oligos using default settings.
...
The man page (`?oligos`) contains a detailed description of each variable. What may not be entirely self-explanatory is that some variables (`sequence` `gcContent`, `tm` and `deltaG`) can hold several values in each row. All values within a specific row can be retrieved by:   
...
Moreover, each oligo is scored based on their average identity, coverage, degeneracy, GC-content and tm range (see the 'score' variable in the table above). The value can range from 0 to 12, where 0 is best. Detailed information about the scoring system can be found at the man page (`?oligos`).
...
For mixed primers, identity refers to the mean identity of the target alignment at the 5' consensus part and coverage refers to the mean coverage of the target alignment at the 3' degenerate part. Values of 1 indicate perfect complementary to all target variants in the input alignment. 

**Masking**

For sequencing applications, such as genotyping, there is often interest in amplifying a specific region of the genome, which means that primers cannot be placed within this region. In order words, the primer dataset must be masked to not contain forward primers after the intended start position and to not contain reverse primers after the end position of the desired amplicon region.  
...
There are many assay candidates. A way of reducing the number is to only select the assays with the best scores. The score for an assay denotes the total oligo score. Thus, the best possible score is 0 and the worst possible score is 24 for assays with only primers and 36 for assays with probes (since the maximum possible score for each oligo is 12). 
...
To illustrate assay design with modified settings, I use the masked dataset from Step 2 (`masked`). Recall that I wanted an assay with only primers, with an amplicon region covering position 2000-3000.
...
The oligo and assay region(s) can be inspected in more detail by using the consensus profile from Step 1. As an example, I select to take a closer look at the first assay in `myAssaySelection`. 
