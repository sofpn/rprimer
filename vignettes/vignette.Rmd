...
For an oligo to work efficiently, it must bind exclusively to a conserved region of the target of interest and not fold or form stable interactions (primer dimers) with other oligos. This together makes (RT)-PCR assay development challenging, and a large number of computational tools have been developed to aid in the design process [@guo2020classification]. Unfortunately, many of them are based on a single sequence, which means that oligos can be inadvertently placed in sequence heterogeneous regions. This can, in part, be circumvented by making a “consensus profile” from an alignment of the target sequences of interest, and masking the variable regions before the design process. Yet, for many viruses, it can be difficult to find even a few completely conserved regions that are suitable as primer or probe targets. 
...
rprimer seeks to address the challenge of amplifying sequence variable viruses by providing tools for visualizing sequence conservation and generating degenerate oligos from a multiple DNA sequence alignment. In this document, I demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR primers, probes and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
For brevity's sake, and to get the alignment to agree more with the average HEV genome length, I chose to mask all positions with a gap frequency of more than 50 %. But keep in mind that gap masking may lead to placement of primers and probes in insert-rich regions.  
...
Or to visualize the nucleotide distribution, by specifying `type = nucleotide`: 
...
Once the consensus profile is generated, oligos can be designed. Primers must be designed and probes are optional. Primers can be generated using either one of the two following strategies:
...
The man page (`?oligos`) contains a detailed description of each variable. What may not be entirely self-explanatory is that some columns (`sequence` `gcContent`, `tm` and `deltaG`) can hold several values. In such cases, all values within an entry can be retrieved by:   
