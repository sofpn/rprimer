...
rprimer provides tools for analyzing sequence conservation and designing degenerate oligos (primers and probes) from a multiple DNA sequence alignment. The package is developed primarily for sequence variable viruses, but it should also be useful for other targets with high sequence variability.  

In this vignette, I describe and demonstrate how to use the package by designing broadly reactive primers and probes for PCR based amplification and detection of hepatitis E virus (HEV), which is a sequence variable RNA virus and a common foodborne pathogen. 
...
The manual (`?oligos`) contains a detailed description of each variable, but what may not be fully self-explanatory is that some variables (e.g. `sequence` `gcContent` and `tm`) can hold several values in each entry. All values within a specific row can be retrieved by:   
...
All candidate assays are located within a conserved stretch of ~70 bases, which also is the target region for the most frequently used RT-qPCR assay for detection of HEV, developed by Jothikumar and colleagues [@jothikumar2006broadly].

There are now over 1000 assays, but the number can be reduced by subsetting the output dataset by e.g. score (see the score variable in the table above). 
...
Note that all sequences will be written in the same direction as the input target alignment, and that all sequence variants of each oligo will be printed.
