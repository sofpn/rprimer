...
If the degeneracy is to be kept low and the target sequence heterogeneous, it is often difficult to fully avoid mismatches between the template and primers/probe. The effect of a mismatch depends to a great extent on its location. For primers, mismatches at the 3’ end can greatly reduce amplification efficiency and should best be avoided, while mismatches at the 5’ end are generally less detrimental. Mismatches within the hydrolysis probe can reduce the fluorescence signal, especially if located at the 5' end, since the 5’-3’ exonuclease activity of the DNA polymerase relies on completely hybridized nucleotides to hydrolyse DNA duplexes [@Muradrasoli]. **MORE REFS** Mismatching DNA duplexes have lower melting temperature (tm) than their perfectly matching counterparts. The negative effect of mismatches can be reduced by either lowering the annealing temperature or by increasing the primer length (to raise tm), so that the annealing temperature can be kept high enough to avoid primer interaction with non-targets. Hence, with careful design it is possible to efficiently amplify targets even with a few mismatches.

rprimer was developed to **adress**

implement commonly adopted primer and probe design guidelines for sequence variable targets. It is especially developed for finding the most conserved regions of short viral genomes with high sequence heterogeneity. It takes a multiple sequence alignment with target sequences of interest as input and designs primers, probes and PCR assays from user specified design criteria. **The output - degenerate primers/ all variants .... the problem I seek to adress is** **simple**
...
* `temperature` Annealing temperature (in Celcius), for calculation of delta G. Delta G is calculated for perfectly matching oligo-target duplexes using the nearest neighbor method [@SantaLuciaUnified], using formula, salt correction method and table values as described in [@santalucia2004thermodynamics]
...
In this case, identity refers to the mean identity of the target alignment at the 5' consensus part and coverage refers to the mean coverage of the target alignment at the 3' degenerate part. Values of 1 indicate perfect complementary towards all targets in the input alignment. 
...
These objects can then easily be exported in fasta-format, so that they can be used by other software for e.g. multiple sequence alignment or hairpin and dimer check. 
