...
The man page (`?oligos`) and the pdf-manual contain a detailed description of each variable, but what may not be entirely self-explanatory is that some variables (e.g. `sequence` `gcContent` and `tm`) can hold several values in each row. All values within a specific row can be retrieved by:   
...
All valid oligos are scored using a simple system based on average identity, coverage, degeneracy, GC-content and tm range. The value can range from 0 to 12, where 0 is best. Detailed information about the scoring system can be found at the man page (`?oligos`) or in the pdf-manual.
...
Regarding identity and coverage values, there is one important difference between ambiguous and mixed primers: 

* For ambiguous primers (and probes), identity refers to the average identity of the oligo binding region and coverage refers to the average coverage of the oligo binding region
* For mixed primers, identity refers to the average identity within the target alignment at the 5' consensus part of the primer and coverage refers to the average coverage of the target alignment at the 3' degenerate part of the primer

See 'Step 1: `consensusProfile`' for more information on identity and coverage. 
...
* `asIUPAC`: if the oligos should be presented in IUPAC-format or not. Defaults to `TRUE`. If `FALSE`, all the sequence variants of each oligo will be presented instead. 
