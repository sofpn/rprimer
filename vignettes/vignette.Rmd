...
The man page (`?oligos`) contains a detailed description of each variable, but what may not be entirely self-explanatory is that some variables (e.g. `sequence` `gcContent` and `tm`) can hold several values in each row. All values within a specific row can be retrieved by:   
...
All valid oligos are scored using a simple system, which is based on average identity, coverage, degeneracy, GC-content and tm range. The value can range from 0 to 12, where 0 is best. Detailed information about the scoring system can be found at the man page (`?oligos`).
