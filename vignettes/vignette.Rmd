...
rprimer provides tools for visualizing sequence conservation and designing degenerate oligos from a multiple DNA sequence alignment. In this document, I describe and demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR oligos and assays for detection of hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
All sequence variants of an oligo must fulfill all the specified design constraints to be considered. Also, oligos with sequence variants containing more than four consecutive runs of the same nucleotide (e.g. "AAAAA") and/or more than three consecutive runs of the same di-nucleotide (e.g. "TATATATA") are excluded for consideration.
...
Sometimes the amplicon must span a certain genomic region. To accomplish this, it is possible to "mask" the primer dataset, to not place forward primers after the start position and to not place reverse primers after the end position. This is exemplified below: 
...
To get an overview, it is possible to highlight the amplicon region in the consensus profile, using the argument `highlight`:

```{r fig 3, fig.width=12, fig.height=6}
plotData(myConsensusProfile, highlight = c(from, to))
```

I now make a subset of the consensus profile above, so that it only contains the amplicon region: 
