...
In this document, I demonstrate how to use the package by designing broadly-reactive RT-(q/d)PCR primers, probes and assays for hepatitis E virus (HEV), which is a sequence variable RNA-virus and a common foodborne pathogen.  
...
* The **mixed strategy** is based on the Consensus-Degenerate Hybrid Oligonucleotide Primer (CODEHOP) principle. It generates primers from both the majority and the IUPAC consensus sequence, which consist of a shorter degenerate part at the 3' end (~1/3 of the primer) and a longer consensus part at the 5' end (~2/3 of the primer). This means that the 5' end will lack ambiguous bases, and instead contain the most probable nucleotide at each position. **[TANKEN BAKOM DETTA ÄR... DENNA STRATEGI KAN VARA BRA OM MAN VILL AMPLIFIERA VÄLDIGT VARIABLA REGIONER, TEX INFÖR KARAKTERISERING/GENOTYPNING]** Longer primers (> 25 bases) are recommended when using this approach, to provide a sufficiently high tm in spite of mismatches. 
...
**Design of primers using the mixed strategy**

I now select a specified region of interest (position 1000 to 4000) and design primers only, using the mixed strategy. I also modify some of the other settings: 
...
It is possible to subset the oligo dataset in many different ways. Below, I select to look at oligos with an average coverage higher than 0.9:
...
bindingRegion <- myConsensusProfile[myConsensusProfile$position >= selection[1, ]$start & myConsensusProfile$position <= selection[1, ]$end, ]
```
...
I notice that all assays are located in the conserved region between position 5000-5500 **[VILKER SKULLE GÖRA DET OMRÅDET TILL ETT PASSANDE MÅLOMRÅDE FÖR EN DETEKTIONSASSAY; IT IS IN FACT THE MOST WIDELY USED REGION, REF JOTHI]**
...
It is also possible to highlight the amplicon region, using the optional argument `highlight`.
...
rprimer covers several steps in the oligo and assay design processes, but not all. Two very important, additional parts are: 1) checks for the potential of hairpin and primer-dimer formation and: 2) in silico checks for exclusivity, by matching the primer and probe set to non-targets. **[MER HÄR]**
