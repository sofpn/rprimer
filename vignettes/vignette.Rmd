...
Intended use: RNA virus, detektionsassay, typningsassay 
...
* The **mixed strategy** generates primers from both the majority (~2/3 of the primer, 5' end) and IUPAC consensus sequence (~1/3 of the primer, 3' end). This means that the 5' end will lack ambiguous bases, and instead contain the most probable nucleotide at each position. This is a way of reducing the degeneracy for very variable targets, since 5' end mismatches are considered to be less detrimental than 3' end mismatches. This approach is (roughly) based on the Consensus-Degenerate Hybrid Oligonucleotide Primer (CODEHOP) principle... Recommended with long lengths... 

Probes are always designed using the ambiguous strategy. 

**Design settings**

`oligos()` has the following arguments: 
...
I now use the mixed strategy, and design primers only, from a specified region of interest, and modify some of the settings: 
...
.........We can subset oligos, to only get the ones with e.g. highest average coverage: 

```{r, fig.align="center", fig.width=12, fig.height=8}
selection <- myMixedPrimers[myMixedPrimers$coverage > 0.95, ]
...
It is also possible to select an oligo and plot the nucleotide distribution of the binding region, by subsetting the consensus profile from Step 1. I do this for the first primer in the dataset above:

```{r}
## Get the binding region of the first oligo (row 1): 
bindingRegion <- myConsensusProfile[myConsensusProfile$position >= myMixedPrimers[1, ]$start & myConsensusProfile$position <= myMixedPrimers[1, ]$end, ]
