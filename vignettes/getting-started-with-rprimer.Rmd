...
It is often valuable to investigate how the generated oligos match with their target sequences. `checkMatch()` can be used for this purpose. The function is a wrapper to `vcountPDict()` from Biostrings [@PagesBios] and has two arguments: 
...
The sequences can now be saved in fasta-format by using `writeXStringSet()` from Biostrings [@PagesBios].
