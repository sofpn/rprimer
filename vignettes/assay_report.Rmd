...
<br><br>
**Table 1**. General assay information. *Begin:* Where the assay region starts (with regards to the alignment/region of interest). *End:* Where the assay region ends. *Tm difference, primers:* Tm difference between the two primers (absolute value, in C). *Tm difference, probe-primers:* Tm of the probe minus the average Tm of the primer pair. Note that Tm-differences are calculated from majority oligos. They may therefore be misleading for degenerate (IUPAC) oligos. *Degeneracy:* Total number of oligo variants in the assay. *Perfect match:* Proportion of perfectly matching sequences. 
<br>
```{r}
...
### Amplicon region information (consensus sequence)
...
```
<br>
Amplicon sequence: `r consensus`
<br>
Amplicon GC-content: `r round(gc_content(consensus), 2)*100` %
<br>
Presence of G repeats (at least four consecutive G:s): `r find_g_repeats(consensus)` 
<br>
<br><br>

### Match information (in silico specificity)
