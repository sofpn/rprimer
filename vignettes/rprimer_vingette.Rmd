...
Aldready existing functionality can be used to import an alignment 

In this example, I use an alignment with 100 HEV sequences that I have collected from GenBank, which is provided with the package. 
...
myAlignment <- infile %>%
  readDNAMultipleAlignment(., format = "fasta") %>%
  maskGaps(., min.fraction = 0.5, min.block.width = 1)

# Select region of interest
colmask(myAlignment, invert = TRUE) <- IRanges::IRanges(start = 1000, end = 5000)

#
myProfile <- consensusMatrix(myAlignment, as.prob = TRUE) #%>%
