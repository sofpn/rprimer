...
The sequence profile can be visualized with `rp_plot()`. The `rc` option regulates whether the sequence should be displayed as a reverse complement or not (the default is `FALSE`).

```{r, fig.width=6, fig.height=6, fig.align='center'}
# The first 30 bases of the sequence profile  
rp_plot(my_sequence_profile[ , 1:30], rc = FALSE) 
```
...
```{r,fig.width=6, fig.height=6, fig.align='center'}
rp_plot(my_sequence_properties[200:500, ]) # Select position 200-500
```
...
### Step 8: Save results 
Alignments, sequence properties, primer, probe and assay candidates can be exported using `rp_save()`. rprimer_alignment objects will be saved as .txt-files, whereas objects of rprimer_sequence_properties, rprimer_oligo and rprimer_assay objects will be saved as .csv-files.
