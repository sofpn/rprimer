...
```{r setup}
# library(rprimer) #do i have to include magrittr here?
devtools::load_all(".")
```
...
**describe more: tm, perfect matches**
...
# Select a favourite assay
selected_assay <- my_assays[1, ]

# Save it in a detailed html-report 
# write_report(
...
# For brewity, I decided to only include the first 50 sequences in the figure
my_alignment_selection <- my_alignment[1:50]
...
### Step 4: Find primers 
`get_oligos()` takes sequece properties as input and searches for oligos that fulfil the following criteria: 
...
* `max_tm_difference` The maximum Tm difference between the two primers (absolute value, in Celcius). The default is 1. Note that Tm-difference is calculated from the majority oligos, and may thus be misleading for degenerate (IUPAC) oligos.
...
For each assay, a report can be generated by calling `write_report()`. You can either select one assay an generate a report, or generate reports for all assays (see code example below). 
...
# It is also possible to write reports for ALL assays , show \w loop 


```

The assay report contain more detailed information about the assay, and the output is presented in html format, as in the example below. This function requires that RMarkdown and kablEextra are installed.   
<br><br>
![](design_report1.png)
![](design_report2.png)
![](design_report3.png)
