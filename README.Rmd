...
<!-- badges: start -->
![R build status](https://github.com/sofpn/rprimer/workflows/R-CMD-check/badge.svg)
...
rprimer provides tools for visualization of sequence conservation and generating degenerate DNA oligos from a multiple sequence alignment. It is especially developed for sequence variable viruses. 
...
The first step is to import an alignment with target sequences of interest and, if preferred, mask positions with high gap frequency. Please use the `readDNAMultipleAlignment()` and `maskGaps()` from the Biostrings for this part. 
...
The nucleotide distribution can be shown, preferably within a short range, by specifying `type = "nucleotide`:
...
The next step is to design oligos. You can either use the default settings as below, or adjust the design constraints (see the package vignette or `?oligos` for more information).
