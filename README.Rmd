...
rprimer provides tools for designing broadly reactive primers, probes and (RT)-(q/dd)PCR assays from a multiple DNA sequence alignment. 
...
`assays()` finds pairs of forward and reverse primers and combines them with probes, if `probe = TRUE` was used in the oligo design step. You can either use default settings (as below), or customize the amplicon length and maximum allowed difference between the primers, and between the primer pair and probe. 
