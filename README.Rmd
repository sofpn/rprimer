...
The nucleotide distribution can be shown, preferably on a short range, by specifying `type = "nucleotide`:
...
The next step is to design oligos. You can either use the default settings as below, or adjust the design constraints (see the package vignette or `?rprimer::oligos` for more information).
...
`assays()` finds pairs of forward and reverse primers and combines them with probes, if probes are present in the dataset. You can either use the default settings as below, or adjust the design constraints (see the package vignette or `?rprimer::assays` for more information).
