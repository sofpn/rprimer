...
combine_match_matrices <- function(x) {
  ...
  match_matrix <- purrr::map(seq_len(nrow(x)), function(y) {
    ...
    majority_matr <- match[, c(1, 3)]
    if (!is.matrix(majority_matr)) {
      majority_matr <- matrix(majority_matr, ncol = 2)
    }
    match_majority_all <- rowSums(majority_matr)
    iupac_matr <- match[, c(2, 4)]
    if (!is.matrix(iupac_matr)) {
      iupac_matr <- matrix(iupac_matr, ncol = 2)
    }
    match_iupac_all <- rowSums(iupac_matr)
    match_majority_all <- ifelse(match_majority_all == 2, TRUE, FALSE)
    match_iupac_all <- ifelse(match_iupac_all == 2, TRUE, FALSE)
  ...
  names(match_percentage) <- c("pm_majority_all", "pm_iupac_all")
