...
# The code was written by Sofia in R 4.0.0
...
#' sequences.
#' \code{read_fasta_alignment} is a wrapper around the base function
#' \code{readLines}.
#'
#' @param x The name of the file (a character vector of length one) which the
#' alignment is to be read from. The file should contain one or more aligned
#' DNA sequences in fasta format. Sequences in fasta format
#' always begins with a '>' symbol, followed by a single-line name.
#' The sequence data is always present on the next line.
#' All sequences (including gaps) must be of the same length.
#'
#' @details The file must contain a DNA alignment of at least 200 bases,
#' and at least one sequence. All sequences must have unique names.
#' Valid nucleotides are 'a', 'c', 'g', 't', 'r', 'y', 'm', 'k', 's', 'w',
#' n', 'h', 'd', 'v', 'b' and '-'.
#'
#' @return A named list  containing the aligned DNA sequences from the input
#' file. Each object (DNA sequence) has the same name as its
#' sequence name in the input file, except for the '>' symbol. Each DNA
#' sequence is a character vector of length one, in lowercase format.
#' The list has the class attribute 'rprimer_alignment'.
#'
#' @examples
...
read_fasta_alignment <- function(x) {
  if (!is.character(x) || length(x) != 1) {
    stop("A filename (character vector of length one) is expected",
         call. = FALSE)
  }
  ...
  # (sequence names in fasta files always begin with ">")
  index <- grep(">", infile)
  # Stop if the file does not appear to be in fasta format
  if (length(index) == 0L) {
    stop("The file does not appear to be in fasta format
      (no line starts with '>').", call. = FALSE)
  }
  if (length(index) < 1L) {
    stop("The file must contain at least one sequence.", call. = FALSE)
  }
  # Identify where the sequences start
  # (which is at the row just below the sequence name)
  begin <- index + 1
  # Identify where the sequences end
  # (which is at the row just above the sequence name)
  end <- index - 1
  # Remove the first "end", as there is no sequence above
  # the first sequence, and add the last "end", which is
  # where the infile ends
...
#' Remove positions with gaps in an alignment
#'
...
#'
#' # Warning
#' \code{remove_gaps} should be used with caution during oligo and assay
#' design. Removing positions with high gap frequency may lead to
#' placement of primers or probes in insertion-rich regions.
#'
#' @return An alignment (an object of class 'rprimer_alignment'), where
#' all positions with a gap frequency higher than the stated threshold
#' are removed.
#'
#' @examples
...
remove_gaps <- function(x, threshold = 0.5) {
  if (!(threshold >= 0.5 && threshold < 1)) {
    stop(paste0(
      "The threshold was set to ", threshold,
       ". Valid threshold values are between 0.5 and <1"), call. = FALSE)
  }
  ...
  # Calculate the gap frequency
  gapfrequency <- colMeans(M)
  # Get the indexes of the positions that should be kept in the sequences
  keep <- which(gapfrequency <= threshold)
  # Keep these positions
  splitted <- purrr::map(splitted, ~ .x[keep])
...
#' (an object of class 'rprimer_alignment').
#'
#' @param from Where the roi begins (an integer).
#'
#' @param to Where the roi ends (an integer, greater than from).
#'
...
select_roi <- function(x, from = 1, to = NULL) {
  ...
  if (!(
    is.numeric(from) && is.numeric(to) && from >= 1 && to > from + 199
    )) {
    stop(
      "from must be at least 1 and to must be at least 'from + 199'",
      call. = FALSE
    )
    if (to > aln_length) to <- aln_length
  }
...
#' abundance (proportion) of each nucleotide at each position in an alignment
#' of DNA sequences.
#'
...
#' Biochemistry, 34: 10581–10594 (Duplex initiation parameters are from here)
#'
...
get_oligos <- function(
  ...
  if (!(
    max_gap_frequency >= 0 && max_gap_frequency <= 1
    && is.numeric(max_gap_frequency) && length(max_gap_frequency) == 1)) {
    stop(
      "max_gap_frequency must be between 0 and 1",
      call. = FALSE)
  }
  if (
    !(max_degenerates <= 30 && max_degenerates >= 0 &&
      is.numeric(max_degenerates))
  ) {
    stop("max_degenerates must be between 0 and 6", call. = FALSE)
  }
  if (!(
    max_degeneracy >= 1 && max_degeneracy <= 64
    && length(max_degeneracy == 1) && is.numeric(max_degeneracy))) {
    stop("max_degeneracy must be between 1 and 64", call. = FALSE)
  }
  if (
    !(min(gc_range) >= 0 && max(gc_range) <= 1 && is.numeric(gc_range))
  ) {
  ...
  if (!(
    is.logical(avoid_3end_ta) && is.logical(avoid_5end_g) &&
    is.logical(avoid_3end_runs))
    ) {
    stop(
  ...
  if (!is.double(conc_oligo) || conc_oligo < 0.2e-07 || conc_oligo > 2e-06) {
  ...
  all_oligos <- purrr::map_dfr(length, function(y) {
    ...
    oligos <- tibble::tibble(
      begin, end, length, majority, iupac, majority_rc, iupac_rc,
      degenerates, degeneracy
    )
    # Exclude oligos with positions with a gap frequency higher than the stated
    # threshold
    ...
    oligos <- oligos[
      which(gc_majority >= min(gc_range) & gc_majority <= max(gc_range))
      , ]
    ...
    oligos <- oligos[
      which(tm_majority >= min(tm_range) & tm_majority <= max(tm_range))
      , ]
    # Indentify non-complex oligos and (if specified) oligos
    # with 3' t-a or 5' g, and set to NA
    oligos$majority <- exclude_unwanted_oligos(
      oligos$majority, avoid_3end_ta = avoid_3end_ta,
      avoid_5end_g = avoid_5end_g, avoid_3end_runs = avoid_3end_runs
    )
    oligos$majority_rc <-  exclude_unwanted_oligos(
      oligos$majority_rc, avoid_3end_ta = avoid_3end_ta,
      avoid_5end_g = avoid_5end_g, avoid_3end_runs = avoid_3end_runs
    )
    ...
    invalid_oligos <- dplyr::intersect(
      which(is.na(oligos$majority)), which(is.na(oligos$majority_rc))
    )
    if (length(invalid_oligos) > 0L) oligos <- oligos[-invalid_oligos, ]
    return(oligos)
  })
  if (nrow(all_oligos) == 0L) stop("No oligos were found.", call. = FALSE)
...
get_assays <- function(
  x, length = 65:120, max_tm_difference = 1
  ) {
  if (!inherits(x, "rprimer_oligo")) {
    stop("An rprimer_oligo object is expected.", call. = FALSE)
  }
  if (
    !(max_tm_difference > 0 && max_tm_difference < 30
      && is.numeric(max_tm_difference))
   ) {
    stop("max_tm_difference must be between 0 and 30", call. = FALSE)
  }
  if (
    !(
      min(length) >= 40 && max(length) <= 5000 && is.numeric(length))
  ) {
  ...
  tm_difference_primer <- abs(
    assays$tm_majority_fwd - assays$tm_majority_rev
  )
  ...
  drop <- c(
    "majority_rc_fwd", "iupac_rc_fwd", "majority_rev",
    "iupac_rev"
  )
  ...
  assays <- assays[
    which(
      assays$amplicon_length >= min(length) &
        assays$amplicon_length <= max(length)
    ),
  ]
  assays <- assays[
    which(assays$tm_difference_primer <= max_tm_difference),
  ]
  if (nrow(assays) == 0L) stop("No assays were found.", call. = FALSE)
...
add_probes <- function(x, y, tm_difference = c(0, 20)) {
  ...
  if (
    !(
      min(tm_difference) >= -20 && max(tm_difference) <= 20 &&
      is.numeric(tm_difference)
     )
  ) {
    stop(
      "tm_difference must be between -30 and 30, e.g. c(-1, 5)", call. = FALSE
    )
  }
  assays <- x
  probes <- y
  ...
  # For assays that has n number of probes, we need to repeat that row n times.
  ...
  if (nrow(assays) == 0L) stop("No assays could be generated.", call. = FALSE)
  # Now, we can make a data frame of the probe candidates
  probe_candidates <- do.call("rbind", probe_candidates)
  # Check the sense of the probe, if the plus sense probe is ok,
  # (i.e. not NA), we take that one.
  ...
  probe_candidates <- probe_candidates[
    , -which(names(probe_candidates) %in% drop)
  ]
  ...
  tm_difference_primer_probe <- purrr::map_dbl(
    seq_len(
      nrow(assays)), ~ assays$tm_majority_pr[[.x]] -
      mean(assays$tm_majority_fwd[[.x]], assays$tm_majority_rev[[.x]]))
  assays <- tibble::add_column(
    assays, tm_difference_primer_probe, .after = "tm_difference_primer"
  )
  assays <- assays[
    which(
      assays$tm_difference_primer_probe >= min(tm_difference) &
      assays$tm_difference_primer_probe <= max(tm_difference)
  ), ]
  if (nrow(assays) == 0L) stop("No assays could be generated.", call. = FALSE)
  assays <- tibble::new_tibble(assays, class = "rprimer_assay")
  return(assays)
...
check_match.rprimer_oligo <- function(x, y) {
  # If the positive-sense sequence is NA, then take the negative sense
  # and make a reverse complement
  # Make regular expressions of majority and iupac sequences
  majority <- ifelse(
  ...
  iupac <- ifelse(
    !is.na(x$iupac), x$iupac,
    purrr::map_chr(x$iupac_rc, reverse_complement)
  )
  ...
  # Make a matrix that describes exactly which
  # sequences the oligo matches perfectly to
...
check_match.rprimer_assay <- function(x, y) {
  ...
  names(match_matrix) <- paste0(
    x$majority_fwd, "_", x$majority_rev, "_", x$majority_pr
  )
  match_percentage <- purrr::map(match_matrix, colMeans)
  match_percentage <- do.call("rbind", match_percentage)
  match_percentage <- tibble::as_tibble(match_percentage)
...
rp_plot.rprimer_sequence_profile <- function(
  ...
  if (!(from >= 1 && to >= 1)) {
    stop(
      "from and to must be 1 or greater",
       call. = FALSE
    )
  }
  if (!(is.logical(rc))) stop("rc must be set to TRUE or FALSE", call. = FALSE)
...
write_assay_report <- function(
  ...
  if (!requireNamespace("rmarkdown", quietly = TRUE)) {
    stop(
      "rmkardown is needed for this function to work. Please install it.",
      call. = FALSE
    )
  }
  if (!requireNamespace("kableExtra", quietly = TRUE)) {
    stop(
      "kableExtra is needed for this function to work. Please install it.",
      call. = FALSE
    )
  }
